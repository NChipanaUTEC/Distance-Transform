...
void* runThreads(void* th){
  long tid;
   ...
   Par *par=v[tid];
   for(int z=0;z<image.image_size.depth;z++){
   cout<<par->inicio<<" "<<par->final<<" "<<par->mnd<<endl;
     for(int x = par->inicio; x < par->final; x++) {
             if(image[image.image_size(x,0,z)] == 0) {
...
int main(){
        ...
        for (size_t z = 0; z < dZ; z++) {
          for(int x = 0; x < mX; x++) {
                  if(image[image.image_size(x,0,z)] == 0) {
                          image2[image2.image_size(x,0,z)]=0;
                  }
                  else{
                          image2[image2.image_size(x,0,z)]=inf;
                  }
                  for(int y = 1; y < nY; y++) {
                          if(image[image.image_size(x,y,z)] == 0) {
                                  image2[image2.image_size(x,y,z)]=0;
                          }
                          else{
                                  image2[image2.image_size(x,y,z)]=1 + image2[image2.image_size(x,y-1,z)];
                          }
                  }
                  for(int y = nY-2; y >= 0; y--) {
                          if(image2[image2.image_size(x,y+1,z)]<image2[image2.image_size(x,y,z)]) {
                                  image2[image2.image_size(x,y,z)]=1+image2[image2.image_size(x,y+1,z)];
                          }
                  }
          }
        ...
        for (size_t z = 0; z < dZ; z++) {
          for(int y = 0; y < nY; y++) {
                  ...
                  t[0] = 0;
                  for(int u = 1; u < mX; u++) {
                          while(q>= 0 && euclidean(t[q],s[q],image2[image2.image_size(s[q],y,z)])>euclidean(t[q],u,image2[image2.image_size(u,y,z)]))
                                  q--;
                          ...
                          if(q < 0) {
                                  ...
                                  w = 1 + sep(s[q],u,image2[image2.image_size(s[q],y,z)],image2[image2.image_size(u,y,z)]);
                                  if(w < mX) {
                                          q++;
                                          s[q] = u;
                                          t[q] = w;
                  ...
                  for(int u = mX-1; u >= 0; u--) {
                          image3[image3.image_size(u,y,z)]=floor(sqrt(euclidean(u,s[q],image2[image2.image_size(s[q],y,z)])));
                          if(u == t[q]) {
                                  q--;
                          }
                  }
        ...
        time_taken2 *= 1e-9;
        cout << "Time taken by program is : " << fixed << time_taken2 << setprecision(9)<< " sec" << endl;
